...
mender_err_t mender_net_get_host_port_url(const char *path, const char *config_host, char **host,
					  char **port, char **url)
{
	...
	/* Check if the path start with protocol (meaning we have the full path); alternatively we
	 * have only URL (path/to/resource) */
	if ((false == mender_utils_strbeginswith(path, "http://")) &&
	    (false == mender_utils_strbeginswith(path, "https://"))) {

		/* Path contains the URL only, retrieve host and port from configuration
		 * (config_host) */
		assert(NULL != url);
	...
	/* Extract url path: next '/' character in the path after finding protocol must be the
	 * beginning of url */
...
mender_err_t header_add(const char **header_list, size_t header_list_size, const char *header)
{
	...
	// Headers are added to the header list one by one so that there are no empty spaces in the
	// list
	if (NULL == header_list) {
		return MENDER_FAIL;
	}

	// The list that we pass to the Zephyr request needs to be NULL-terminated so the last
	// element need to stay NULL
...
int mender_net_connect(const char *host, const char *port)
{
	...
	do {
		...
		mender_log_debug("Unable to resolve host name '%s:%s': %s", host, port,
	...
	if (0 != result) {
		mender_log_error("Unable to resolve host name '%s:%s': %s", host, port,
				 zsock_gai_strerror(result));
		goto END;
	}
...
#endif /* CONFIG_NET_SOCKETS_SOCKOPT_TLS */
		mender_log_error("Unable to create socket, result = %d, error: %s", sock,
				 strerror(errno));
		goto END;
	}
	...
	if ((result = zsock_setsockopt(sock, SOL_TLS, TLS_SEC_TAG_LIST, sec_tag, sizeof(sec_tag))) <
	    0) {
		mender_log_error("Unable to set TLS_SEC_TAG_LIST option, result = %d, error: %s",
	...
	if ((result = zsock_setsockopt(sock, SOL_TLS, TLS_HOSTNAME, host, strlen(host))) < 0) {
		mender_log_error("Unable to set TLS_HOSTNAME option, result = %d, error: %s",
				 result, strerror(errno));
		goto END;
	}
	...
	if ((result = zsock_setsockopt(sock, SOL_TLS, TLS_PEER_VERIFY, &verify, sizeof(int))) < 0) {
		mender_log_error("Unable to set TLS_PEER_VERIFY option, result = %d, error: %s",
				 result, strerror(errno));
		goto END;
	}
	...
	if (0 != (result = zsock_connect(sock, addr->ai_addr, addr->ai_addrlen))) {
		mender_log_error("Connect failed: host=%s port=%s rc=%d errno=%d", host, port,
				 result, errno);
		goto END;
	}
