 ...
 */
#define MENDER_HEADER_HTTP_USER_AGENT                                                              \
	"User-Agent: Mender/" MENDER_CLIENT_VERSION " MCU Zephyr/" KERNEL_VERSION_STRING "\r\n"
...
/*
 * nRF91 modem TLS sockets are limited to roughly 2 KB of decrypted payload at
 * once. Nordic's downloader library works around this by using bounded HTTP
 * range requests for HTTPS transfers.
 */
#define MENDER_HTTP_ARTIFACT_RANGE_MAX 2048

/*
 * Artifact downloads over the modem-backed TLS socket can deliver fragments
 * larger than 512 bytes, which causes Zephyr's HTTP client to abort with
 * EMSGSIZE mid-stream. Use a larger receive buffer for OTA downloads.
 */
const size_t mender_http_recv_buf_length = 4096;

/**
 * @brief Request context
 */
typedef struct {
	mender_err_t (*callback)(mender_http_client_event_t, void *, size_t,
				 void *); /**< Callback to be invoked when data are received */
 ...
 */
static HTTP_CALLBACK_RETURN_TYPE http_response_cb(struct http_response *response,
						  enum http_final_call final_call, void *user_data);
 ...
 * @return HTTP_CALLBACK_RETURN_VALUE, 0 if Zephyr >= 4.2.0, void otherwise
 */
static HTTP_CALLBACK_RETURN_TYPE artifact_response_cb(struct http_response *response,
 ...
 */
static int parse_http_date(const char *str, struct tm *tm)
{
	...
	if (7 !=
	    sscanf(str, "%3s, %d %3s %d %d:%d:%d", day_name, &day, mon, &year, &hour, &min, &sec)) {
...
static int on_header_value(struct http_parser *parser, const char *at, size_t length)
{
...
#ifdef CONFIG_MENDER_HTTP_PARSE_DATE_HEADER
	/* We're not guaranteed to have the correct time set on the device, so we attempt to parse
	 * the Date header on a response instead. According to
	 * https://httpwg.org/specs/rfc9110.html#field.date, the Date header is sent on 4xx (Client
	 * Error) responses as long as the server has a clock. If we fail to parse the header and
	 * the server responds with an HTTP-date in Retry-After, we will fall back to using regular
	 * backoff intervals */
	if (ctx->parsing_date) {
	...
	if (ctx->parsing_retry_after) {
		...
		if ((string == endptr) || ('\0' != *endptr) || (ERANGE == errno) ||
		    (0 == seconds)) {
#ifdef CONFIG_MENDER_HTTP_PARSE_DATE_HEADER
			/* Try to parse the Retry-After value as an HTTP-date. We compare the
			 * Retry-After HTTP-date with the HTTP-date from the Date header of the HTTP
			 * response. Normally this won't be the case, as Mender Server returns the
...
*/
mender_err_t mender_http_perform(char *jwt, char *path, mender_http_method_t method, char *payload,
				 char *signature,
				 mender_err_t (*callback)(mender_http_client_event_t, void *,
							  size_t, void *),
				 void *params, int *status)
{
	...
	mender_http_request_context request_context = {
		.callback = callback, .params = params, .ret = MENDER_OK};
	const char *header_fields[10] = {
		NULL}; /* The list is NULL terminated; make sure the size reflects it */
	...
	/* Add headers */
	if (MENDER_FAIL ==
	    header_add(header_fields, header_fields_size, MENDER_HEADER_HTTP_USER_AGENT)) {
		mender_log_error("Unable to add 'User-Agent' header");
		goto END;
	}

	if (NULL != jwt) {
		auth_header = header_alloc_and_add(header_fields, header_fields_size,
	...
	if (NULL != signature) {
		signature_header = header_alloc_and_add(header_fields, header_fields_size,
							"X-MEN-Signature: %s\r\n", signature);
	...
	if (NULL != payload) {
		if (MENDER_FAIL == header_add(header_fields, header_fields_size,
					      "Content-Type: application/json\r\n")) {
	...
	if (MENDER_OK != (ret = callback(MENDER_HTTP_EVENT_CONNECTED, NULL, 0, params))) {
		mender_log_error(
			"An error occurred while calling 'MENDER_HTTP_EVENT_CONNECTED' callback");
		goto END;
	}

	/* Perform HTTP request */
	if ((http_req_ret = http_client_req(sock, &request, MENDER_HTTP_REQUEST_TIMEOUT,
					    (void *)&request_context)) < 0) {
	...
	if (MENDER_OK != (ret = callback(MENDER_HTTP_EVENT_DISCONNECTED, NULL, 0, params))) {
		mender_log_error("An error occurred while calling 'MENDER_HTTP_EVENT_DISCONNECTED' "
				 "callback");
		goto END;
	}
...
mender_err_t mender_http_artifact_download(const char *uri,
					   mender_artifact_download_data_t *dl_data, int *status)
{
	assert(NULL != dl_data);
	assert(NULL != status);
	...
	if (MENDER_OK != (ret = dl_data->artifact_download_callback(MENDER_HTTP_EVENT_CONNECTED,
								    NULL, 0, dl_data))) {
		mender_log_error("An error occurred while calling 'MENDER_HTTP_EVENT_CONNECTED' "
				 "artifact callback");
		goto END;
	}
	connected = true;
	...
	while (true) {
		...
		if (NULL ==
		    (request.recv_buf = (uint8_t *)mender_malloc(mender_http_recv_buf_length))) {
			mender_log_error("Unable to allocate memory");
			goto END;
		}
		request.recv_buf_len = mender_http_recv_buf_length;
		...
		if (use_ranges) {
			...
			range_header =
				header_alloc_and_add(header_fields, header_fields_size,
						     "Range: bytes=%zu-%zu\r\n", offset, range_end);
			if (NULL == range_header) {
		...
		for (int attempt = 1; attempt <= MENDER_HTTP_CHUNK_RETRY_MAX; attempt++) {
			sock = mender_net_connect(host, port);
			if (sock < 0) {
				mender_log_warning("Unable to open HTTP client connection "
						   "(attempt %d/%d)",
						   attempt, MENDER_HTTP_CHUNK_RETRY_MAX);
			} else {
				http_req_ret = http_client_req(
					sock, &request, MENDER_HTTP_REQUEST_TIMEOUT, dl_data);
				mender_net_disconnect(sock);
				sock = -1;
				if (http_req_ret >= 0) {
					break;
				...
				mender_log_warning(
					"HTTP request failed at offset %zu: %s (attempt %d/%d)",
					offset, strerror(-http_req_ret), attempt,
					MENDER_HTTP_CHUNK_RETRY_MAX);
			}
		...
		if (http_req_ret < 0) {
			mender_log_error("HTTP request failed after retries: %s",
					 strerror(-http_req_ret));
			mender_free(request.recv_buf);
			goto END;
		}
		...
		if (0 == request.internal.response.http_status_code) {
			mender_log_error("An error occurred, connection has been closed");
			dl_data->artifact_download_callback(MENDER_HTTP_EVENT_ERROR, NULL, 0,
							    dl_data);
			mender_free(request.recv_buf);
			goto END;
		}
		...
		*status = request.internal.response.http_status_code;
		...
		mender_free(range_header);
		range_header = NULL;
		mender_free(request.recv_buf);

		if (use_ranges) {
			if (206 != *status) {
				mender_log_error("Artifact server ignored range request, status=%d",
						 *status);
				ret = MENDER_FAIL;
				goto END;
			}
			if ((0 == total_size) &&
			    request.internal.response.content_range.total > 0) {
				total_size = request.internal.response.content_range.total;
			}
			if (0 == body_bytes_received) {
				mender_log_error(
					"Artifact server returned empty body for range request");
				ret = MENDER_FAIL;
				goto END;
			}
	...
	if (MENDER_OK != (ret = dl_data->artifact_download_callback(MENDER_HTTP_EVENT_DISCONNECTED,
								    NULL, 0, dl_data))) {
		mender_log_error("An error occurred while calling 'MENDER_HTTP_EVENT_DISCONNECTED' "
				 "artifact callback");
		goto END;
	...
	/*
	 * mender-mcu core expects HTTP 200 for successful downloads. Our modem
	 * transport uses ranged GETs and gets HTTP 206 by design, so normalize
	 * here to keep module sources unpatched.
	 */
...
static HTTP_CALLBACK_RETURN_TYPE http_response_cb(struct http_response *response,
						  MENDER_ARG_UNUSED enum http_final_call final_call,
						  void *user_data)
{
	...
	if ((true == response->body_found) && (NULL != response->body_frag_start) &&
	    (0 != response->body_frag_len) && (MENDER_OK == request_context->ret)) {

		/* Transmit data received to the upper layer */
		if (MENDER_OK !=
		    (request_context->ret = request_context->callback(
			     MENDER_HTTP_EVENT_DATA_RECEIVED, (void *)response->body_frag_start,
			     response->body_frag_len, request_context->params))) {
...
artifact_response_cb(struct http_response *response,
		     MENDER_ARG_UNUSED enum http_final_call final_call, void *user_data)
{
	...
	/* Check if data is available */
	if (response->body_found && (NULL != response->body_frag_start) &&
	    (0 != response->body_frag_len) && (MENDER_OK == (dl_data->ret))) {
		/* Transmit data received to the upper layer */
		dl_data->ret = dl_data->artifact_download_callback(
			MENDER_HTTP_EVENT_DATA_RECEIVED, (void *)response->body_frag_start,
			response->body_frag_len, dl_data);
		if (MENDER_OK != (dl_data->ret)) {
			mender_log_error("An error occurred, stop reading data");
		}
	}
